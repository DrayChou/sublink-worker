...
| 问题 | 原因 | 修复 |
|------|------|------|
| `data.charCodeAt is not a function` | `TextEncoder().encode()` 返回 `Uint8Array`，应使用 `data[i]` | `cacheManager.js:75` |
| D1 初始化失败 `Cannot read properties of undefined` | D1 binding 返回不完整对象 | 添加 `db.exec` 函数存在性检查 |
| 403 Forbidden | 缺少 Referer 头 | 添加动态 `Referer` 和 `Accept` 请求头 |
| `customRules.reverse()` 崩溃 | `customRules` 可能为 null | 添加 `customRules = customRules \|\| []` |
| 缓存未命中 (key 不匹配) | 插入脚本使用 MD5，查询用自定义哈希 | 统一使用相同算法 |

### 11.2 功能增强

| 功能 | 说明 |
|------|------|
| 完整 Clash 配置处理 | 检测到包含 `proxy-groups/rules` 的完整配置时，只提取 `proxies`，避免重复的代理组 |
| D1 调试日志 | 添加详细日志便于排查问题 |
| 缓存脚本统一 | `scripts/cache.js` 整合 insert/fix/delete/list/stats 功能 |

### 11.3 代码变更

```
scripts/cache.js    | 228 +++++++++ (新增 - 统一缓存管理脚本)
src/ProxyParsers.js |  +14 (完整 Clash 配置检测)
src/cacheManager.js |  +21 (错误处理、调试日志)
src/config.js       |   +1 (customRules 空值检查)
```
...
#### Q: 订阅返回完整 Clash 配置，合并后出现多个 proxy-groups？
A: **已修复**。代码会检测完整配置（包含 `proxy-groups` 或 `rules`），只提取 `proxies` 节点，丢弃原始的代理组定义。

#### Q: 403 Forbidden 无法获取缓存？
A: 尝试在 D1 中手动插入缓存：
```bash
# 先在能访问的机器下载订阅
curl -o subscription.txt "https://example.com/sub?token=xxx"

# 插入到 D1
node scripts/cache.js fix "https://example.com/sub?token=xxx" subscription.txt
```

#### Q: 缓存未命中？
A: 检查 cache key 算法是否一致：
```bash
# 验证 key
node scripts/cache.js list
```
